...
#define t 40000000
#define node_num 30 //0 ----> node_num-1#define pkt 5
int h = 500, macMinBE = 3, macMaxBE = 8, m_b = 4, n_b = 1;
int qStateInd[node_num], csmaStateInd[node_num];
int wakeCount[node_num];
int qSlot[node_num];
int w[node_num], m[node_num], n[node_num];
int inB000[node_num], channel[node_num][pkt+1];
...
FILE *fp_r, *fp_al, *fp_be, *fp_booo, *fp_Pwo, *fp_T, *fp_debug;
...
int randBackOffSlot(int m) {
	return power(2, MIN(macMaxBE, macMinBE + m)) * ((float)rand() / (float)(RAND_MAX+1));
...
void debugGen(int time){
	int i=0;
	for(i=0;i<node_num;i++) {
		fprintf(fp_debug, "{t:%d|I:%d|I_N:%d|C:%d|m:%d|w:%d|n:%d|ch:%d|be:%d|fai:%d}\t",time,qStateInd[i],qSlot[i],csmaStateInd[i],m[i],w[i],n[i],channel[i][0],in_beta[i],fail_beta[i]);
	}
	fprintf(fp_debug, "\n");
}
